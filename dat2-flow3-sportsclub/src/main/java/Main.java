...
public class Main {
    ...
    private final static String PASSWORD = "cph-ma670";
    ...
    public static void main(String[] args) {
        ...
        List<Member> members = memberMapper.getAllMembers();
            ...
            ShowSportsWithTeamsCount(memberMapper);
        /*
            ShowTeamsAndMembersCount(memberMapper);
            showMembers(members);
    ...
    private static void ShowTeamsAndMembersCount(MemberMapper memberMapper)
    {
        List<List<Object>> teamsWithMembersCount = memberMapper.getTeamMembersCount();

        for (List<Object> teamWithMembersCount : teamsWithMembersCount)
        {
            System.out.println(teamWithMembersCount.get(0) + " has " + teamWithMembersCount.get(1) + " team members");
        }
    }

    private static void ShowSportsWithTeamsCount(MemberMapper memberMapper)
        {
            List<List<Object>> sportsWithTeamsCount = memberMapper.getSportsWithTeams();

            for (List<Object> sportWithTeamsCount : sportsWithTeamsCount)
            {
                System.out.println(sportWithTeamsCount.get(0) + " has " + sportWithTeamsCount.get(1) + " teams");
            }
        }
